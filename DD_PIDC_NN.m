%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Algorithm Name: Dig  Deeper with Proportional–Integral–Derivative–Cumulative Neural Network (DD-PIDC-NN), also called (DD-MinerNet).
...
% Publish Date: Tuesday, March 19, 2024.  
...
folder1 = '/home/refat/Documents/MATLAB/Data/Coal_resize(240_54)';    %You can define the path of a folder 1 to access the images of coal. 
folder2 = '/home/refat/Documents/MATLAB/Data/Gangue_resize(240_54)';  % You can define the path of a folder 2 to access the images of gangue. 
...
Kp = 1*10^-4;          % proportional (KP)
Ki = 5*10^-4;          % Integral (KI)
Kd = 5*10^-4;          % derivative (KD)
dt = 0.1;              % sampling time (s)
ArbitraryValue = -10;

kernal = cell(4,1);
    kernal{1} = [1 1 1;0 0 0;-1 -1 -1];
    kernal{2} = [0 1 0;1 -4 1; 0 1 0];
    kernal{3} = 0.5.*[1 0 0;0 1 0; 0 0 1];
    kernal{4} = 0.01.*[1 1 0;1 1 1; 0 1 1];
    kernal{5} = (2*rand(3, 3) - 1) * sqrt(6) / sqrt(360 + 2000);
    kernal{6} = (2*rand(3, 3) - 1) * sqrt(6) / sqrt( 10 + 100);
    


    %convolve the image with all kernels

    y = zeros(100,100,6);
    relu_value = zeros(100,100,6);
    pool_value = zeros(50,50,6);
    
    yy = zeros(48,48,4);
    relu_value2 = zeros(48,48,4);
    pool_value2 = zeros(24,24,4);
    
     yyy = zeros(22,22,8);
    relu_value3 = zeros(22,22,8);
    pool_value3 = zeros(11,11,8);
 
    yyyy = zeros(9,9,16);
    %relu_value4 = zeros(9,9,16);
    pool_value4 = zeros(3,3,16);
    %pool_value5 = zeros(1,1,16);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k_0 = 1:3
   for k0 = 1:2
        icz = icz +1;
        y(:,:,icz) = Conv(img(:,:,k_0),kernal{k0});
        pool_value(:,:,icz) = AvgPool(y(:,:,icz),2);
...
icz =0;
     ...
     for k_1=1:6
         for k1 = 3:4
              icz = icz +1;
 yy(:,:, icz)=Conv(pool_value(:,:,k_1),kernal{k1});
 pool_value2(:,:,icz)= AvgPool(yy(:,:, icz),2);
         end
     end
...
icz =0;
     ...
     for k_2=1:12
         for k2 = 5:6
              icz = icz +1;
yyy(:,:,icz)=Conv(pool_value2(:,:,k_2),kernal{k2});
 pool_value3(:,:,icz)= AvgPool(yyy(:,:,icz),2);
         end
     end
...
icz =0;
     ...
     for k_3=1:24
         for k3 = 1:6
              icz = icz +1;
yyyy(:,:,icz)=Conv(pool_value3(:,:,k_3),kernal{k3});
 pool_value4(:,:,icz)= AvgPool(yyyy(:,:,icz),3);
         end
     end
 x = reshape(pool_value4, [], 54);


number_of_neuron =  size(29,1); 
...
C = [F1,F2, F3, F4, F5, F6, F7, F8, F9, F10, F11, F12, F13, F14, F15, F16, F17, F18, F19, F20, F21, F22, F23, F24, F25, F26, F27];
...
% Training
%
Xe = training_data(:, 1:27);
D = training_data(:, 28);

W1 = (2*rand(100, 100) - 1)./ sqrt ((100^2) + (81^2));
W2 = (2*rand(100, 100) - 1);
...
Xv = validation_data(:, 1:27);
Dv = validation_data(:, 28);
accv = 0;
Nv = length(Dv);
